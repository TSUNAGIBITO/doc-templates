...
## 11. 拡張ステークホルダー・ロール権限・認定審査（投資家資料反映）

### 11.1 ロールと権限（`profiles.role`）
| ロール | 対象 | 主な権限 | 対応画面 |
|--------|------|----------|----------|
| `user` | 受験者（一般） | 受験・合格証管理・共有 | SCR-200/201 |
| `creator` | 作成者（企業/クリエイター） | 検定作成・公開・売上・サブスク | SCR-100〜102 |
| `exam_org` | **試験運営団体（新規）** | 既存試験のオンライン配信・採点、受験母数拡大 | SCR-300 試験配信管理 |
| `agency` | **広告代理店（新規）** | 卸値（リセラー）でクライアント検定を代理作成・提案、問題設計コンサル | SCR-310 代理店ダッシュボード |
| `admin` | 運営 | 検定停止・**認定審査**・返金・通報対応 | SCR-900 運営コンソール |

### 11.2 追加画面
| 画面ID | 画面名 | 概要 | 主なロール |
|--------|--------|------|-----------|
| SCR-300 | 試験配信管理 | 既存試験のオンライン配信・採点ジョブ | exam_org |
| SCR-310 | 代理店ダッシュボード | 担当クライアント・卸値・代理作成 | agency |
| SCR-900 | 運営コンソール（認定審査） | 認定/未認定の審査・承認、検定停止 | admin |

### 11.3 認定審査フロー（認定/未認定・要件1.5）
```mermaid
flowchart LR
  A[作成者: 認定申請] --> B[運営: 審査 SCR-900]
  B -->|承認| C[exams.certified=true / certified_at 記録]
  B -->|却下| D[未認定のまま/差戻し]
  C --> E[認定バッジ表示・信頼度UP]
  C --> F[将来: 発行者 issuer の信頼度に反映 ⑥]
```

### 11.4 収益方式による決済分岐（`exams.monetization`）
- `transaction`：受験料決済（API-301）＋20%控除。
- `badge_fee`：合格時にバッジ発行料¥200決済（API-303）。
- `free`：課金なし（無料検定）。

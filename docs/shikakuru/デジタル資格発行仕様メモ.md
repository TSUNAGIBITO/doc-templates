...
> 将来構想（要件定義 REQ-SKR-001 ⑥）とDB設計（DB-SKR-001 §10）の補足。**MVPでは実装せず、M2以降で段階導入**する。
...
| 段階 | 実施 |
|------|------|
| MVP | 自社署名の合格証＋検証ページ。`credentials`を正規化して蓄積（VC-readyだけ確保） |
| M2 | **OB3.0 / W3C VC 発行**、DID・失効リスト。まだ本人確認なしでも詐称困難に |
| M3 | **JPKI eKYC**で「本人確認済みバッジ」。採用・国家資格連動を解禁 |
| 任意 | VCハッシュを公開台帳へアンカリング（PIIは終始オフチェーン） |
